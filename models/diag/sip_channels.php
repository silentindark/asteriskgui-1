    ...
    public function getAll($filter)
    {
        ...
        for ($i = $first + 1; $i < count($arr_rows); $i++) {
            ...
            for ($n = $i; $n < count($arr_rows); $n++) {
                ...
                if (stripos($str, "Extension:") !== false) {
                    $Extension = trim(str_replace('Extension:', '', $str));
                }
                if (stripos($str, "ChannelStateDesc:") !== false) {
                    $ChannelStateDesc = trim(str_replace('ChannelStateDesc:', '', $str));
                }
                if (stripos($str, "BridgedChannel:") !== false) {
                    $BridgedChannel = trim(str_replace('BridgedChannel:', '', $str));
                }
            ...
            //Search
            if (($fChannel) && (stripos($Channel, $fChannel) === false)) {
                $find = false;
            }
            if (($fContext) && (stripos($Context, $fContext) === false)) {
                $find = false;
            }
            if (($fExtension) && (stripos($Extension, $fExtension) === false)) {
                $find = false;
            }
            if (($fChannelStateDesc) && (stripos($ChannelStateDesc, $fChannelStateDesc) === false)) {
                $find = false;
            }
            if (($fBridgedChannel) && (stripos($BridgedChannel, $fBridgedChannel) === false)) {
                $find = false;
            }
            if (($fApplication) && (stripos($Application, $fApplication) === false)) {
                $find = false;
            }
            if (($fApplicationData) && (stripos($ApplicationData, $fApplicationData) === false)) {
                $find = false;
            }
            if (($fCallerIDnum) && (stripos($CallerIDnum, $fCallerIDnum) === false)) {
                $find = false;
            }
            if (($fCallerIDname) && (stripos($CallerIDname, $fCallerIDname) === false)) {
                $find = false;
            }
            if (($fDuration) && (stripos($Duration, $fDuration) === false)) {
                $find = false;
            }
