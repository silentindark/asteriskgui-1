    ...
    public function getAll($filter)
    {
        ...
        for ($i = $first + 1; $i < count($arr_rows); $i++) {
            ...
            for ($n = $i; $n < count($arr_rows); $n++) {
                ...
                if (stripos($str, "IPport:") !== false) {
                    $port = trim(str_replace('IPport:', '', $str));
                }
                if (stripos($str, "Status:") !== false) {
                    $state = trim(str_replace('Status:', '', $str));
                }
                if (stripos($str, "Description:") !== false) {
                    $desc = trim(str_replace('Description:', '', $str));
                }
                $i++;
            ...
            //Search
            if (($fip) && (stripos($ip, $fip) === false)) {
                $find = false;
            }
            if (($fport) && (stripos($port, $fport) === false)) {
                $find = false;
            }
            if (($fstate) && (stripos($state, $fstate) === false)) {
                $find = false;
            }
            if (($fclid) && (stripos($clid, $fclid) === false)) {
                $find = false;
            }
